...
# -*- coding: utf-8 -*-
"""
Rubik's Cube Movement Simulator

Represents a 3x3 Rubik's Cube as a flat list of 54 face-labels and provides
clockwise / anti-clockwise rotation primitives for all six faces, plus
shuffle, reset, and ASCII display utilities.
...
# A single list representing all 54 face-stickers in a serial fashion.
# Colour key: w=white, b=blue, r=red, g=green, o=orange, y=yellow
# Every colour's 5th element is the immovable centre sticker.
r = [
...
def shift(r, L):
    """Cycle 12 stickers in groups of 3 (r is cube list, L is 12-element index array)."""
    t1 = r[L[0]]
    t2 = r[L[1]]
    t3 = r[L[2]]
...
def rotate(r, L):
    """Rotate the 8 stickers of a single face (in-place)."""
    t1 = r[L[0]]
    t2 = r[L[7]]
...
def shuffle(r):
    """Apply a random sequence of 17-32 moves to scramble the cube.

    Bug-fix: the original code eagerly evaluated every move function inside
    the list literal passed to random.choice(), which mutated the cube 12
    times per iteration instead of once.  We now store *references* to the
    functions and call only the chosen one.
    """
    move_funcs = [
        right_c, left_c, up_c, down_c, front_c, back_c,
        back_ac, right_ac, left_ac, up_ac, front_ac, down_ac,
    ]
    for _ in range(random.randint(17, 32)):
        random.choice(move_funcs)(r)
    return r


def reset():
    """Return a fresh solved-state cube list."""
    return real[:]
...
def display(r):
    ...
    Layout (top-to-bottom):
        Blue (top face)
        White (left) | Red (front) | Yellow (right)
        Green (bottom face)
        Orange (back face)

    Bug-fix: row j==6 previously printed r[15] (green g1) instead of r[14]
    (red r3) for the front-face column.
    """
    for j in range(15):
        for i in range(9):
            if j == 0:
